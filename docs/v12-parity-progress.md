...
Combined Character header browser verification: mounted actual CharacterWorkspace on a temporary public local review route; intercepted only v12-fixture API requests with a two-node graph (no production data mutation). Desktop1200 screenshot output/playwright/character-workspace-headings.png inspected: lens→single grouping heading→categories/controls→source medallion/name/kind→direct ledger. Switched All primitives: heading became Every primitive, with its supply paths and Ursa Major supply button displayed. At390px, supply button opened one Kinetic Hardening detail dialog, document width390. Temporary fixture removed/dev stopped. Corrected singular rule count. Production publication of header changes remains pending.

### Library identity and mobile selection follow-up
- Restored stored entity icons in grouped rows and the selected-entry orbit; missing icons use the New Entity defaults. System preview labels now use teal consistently.
