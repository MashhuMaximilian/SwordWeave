  ...
  return <div className="v12-lineage-layout">
    ...
    <div className="v12-network-viewport" ref={viewport} onPointerDown={event => {
      ...
      <div style={{ width: width * zoom, height: height * zoom }}><div className="v12-network-world" style={{ width, height, transform: `scale(${zoom})`, transformOrigin: "0 0" }}>
        <svg width={width} height={height} aria-hidden="true">{session.edges.map((edge, index) => { const from = nodes.find(n => n.node.key === edge.from), to = nodes.find(n => n.node.key === edge.to); if (!from || !to) return null; const x=(from.x+205+to.x)/2,y=(from.y+to.y)/2+32; return <g key={`${edge.from}-${edge.to}-${index}`}><path d={`M ${from.x + 205} ${from.y + 42} C ${from.x + 225} ${from.y + 42}, ${to.x - 25} ${to.y + 42}, ${to.x} ${to.y + 42}`} />{edge.sourceVersionId ? <text x={x} y={y}><title>{edge.sourceVersionId}</title>source {edge.sourceVersionLabel ?? edge.sourceVersionId.slice(0,8)}</text> : null}</g>; })}</svg>
    ...
    <nav className="v12-network-minimap" aria-label="Lineage overview">
      <p className="v12-kicker">Loaded lineage · {nodes.length} {nodes.length === 1 ? "entry" : "entries"}</p>
