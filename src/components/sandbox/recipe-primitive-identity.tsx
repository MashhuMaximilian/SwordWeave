...
export function RecipePrimitiveIdentity({primitive}:{primitive:{id:number;name:string;category:string;buCost?:number;mechanicalOutputText?:string|null;narrativeRule?:string|null}}) {
  ...
  return <button type="button" className="v12-recipe-identity min-w-0 flex-1 text-left" onClick={inspect} aria-label={`View ${primitive.name}`}>
    ...
    <div className="v12-recipe-copy min-w-0">
      ...
      <div className="flex min-w-0 items-start justify-between gap-2">
        <h3 className="min-w-0">{primitive.name}</h3>
        {primitive.buCost !== undefined ? <span className="shrink-0 text-xs text-muted-foreground">{primitive.buCost} BU</span> : null}
      </div>
