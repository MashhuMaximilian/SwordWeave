...
export async function POST(
  ...
  try {
    ...
    const response = await db.transaction(async (tx) => {
      // Ownership + mode gate.
      const [character] = await tx.select().from(characters).where(eq(characters.id, characterId)).for("update");
      if (!character) {
        return NextResponse.json(
      ...
      if (explicitHeritageId) {
        ...
        const templateRow = await tx.query.heritage.findFirst({
        ...
        if (templateRow.kind !== kind) {
          return NextResponse.json({ error: "Heritage kind does not match this accordion." }, { status: 400 });
        }
        ...
        const existing = await tx.query.characterHeritages.findFirst({
          where: and(
        ...
        const existingPrimitives = await tx
        ...
        const [createdHeritage] = await tx
          .insert(heritage)
          .values({
            userId,
            kind,
      ...
      await tx.insert(characterHeritages).values({
        ...
        versionId: await resolveLatestVersionId("template", resolvedHeritageId),
        slotSource: "OWNED",
        isMirrored: false,
        createdAt: new Date(),
      });

      const primitiveCount = await formalizeHeritageBundle(tx, characterId, resolvedHeritageId, userId, character.level);

      await tx.insert(characterLog).values({
        characterId, kind: "heritage_formalized",
        payload: { heritageId: resolvedHeritageId, kind, accordionKind: kind, primitiveCount },
      });
